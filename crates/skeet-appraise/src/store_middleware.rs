...
use skeet_store::{AppraisalsSource, Images, Scores, SkeetStore};

/// The store capabilities the appraisals service needs.
///
/// Aggregates image reads/paging ([`Images`]), score reads ([`Scores`]), and
/// appraisal reads/writes ([`AppraisalsSource`]). Handlers depend on this narrowed
/// surface rather than the whole `SkeetStore`; any type implementing the three
/// ports satisfies it via the blanket impl.
pub trait AppraiseStore: Images + Scores + AppraisalsSource {}
impl<T: Images + Scores + AppraisalsSource> AppraiseStore for T {}
