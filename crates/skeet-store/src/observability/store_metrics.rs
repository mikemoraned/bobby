...
impl StoreMetrics {
    ...
    pub fn record_fragment_counts(&self, counts: &[(TableName, u64)]) {
    ...
    pub fn record_table_versions(&self, versions: &[(TableName, u64)]) {
...
mod tests {
    ...
    fn record_fragment_counts_emits_gauge_per_table() {
        ...
        metrics.record_fragment_counts(&[(TableName::Images, 64), (TableName::Scores, 4)]);
    ...
    fn record_table_versions_emits_gauge_per_table() {
        let (metrics, provider, exporter) = make_test_metrics();
        metrics.record_table_versions(&[(TableName::Images, 42), (TableName::Scores, 7)]);
        let snap = flush_and_collect(&provider, &exporter);
        assert_eq!(
