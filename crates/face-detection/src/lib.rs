...
pub mod model {
    ...
    // Generated by burn_import's ModelGen; we can't edit it to remove its `expect`s.
...
// Overlap areas are non-NaN and `zones` is a fixed 9-element array, so both `expect`s
// below are infallible by construction.
#[allow(clippy::expect_used)]
