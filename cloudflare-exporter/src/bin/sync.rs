...
use clap::Parser;
use cloudflare_exporter::{cloudflare, metrics::SyncMetrics, prom};
use tracing::info;
...
async fn sync(args: &Args) -> Result<u64, Box<dyn std::error::Error>> {
    ...
    for (window_from, window_to) in cloudflare::one_minute_windows(from, to) {
        ...
        let fetched = cloudflare::fetch_r2_metrics(
            &client,
            &args.api_token,
            &args.account_tag,
            window_from,
        ...
        let datapoints = (fetched.operations.len() + fetched.storage.len()) as u64;
        ...
        info!(
            ...
            storage_entries = fetched.storage.len(),
            "fetched and pushing"
        );
