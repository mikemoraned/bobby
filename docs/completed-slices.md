...
Reworked the homepage "images checked" banner from a raw lifetime count into an honest, windowed picture — how many images were examined over a recent period, how few matched, and when the next match is expected — backed by measured statistics rather than a hard-coded guess. No new crates.

- **Prune-statistics storage**: added a `Statistics` port to `skeet-store` (new `prune_stats_v1` lance table) recording per-interval counts of skeets seen, images examined, and images saved with wall-clock bounds; the pruner writes them from a new `content_statistics_stage` split out of the save stage. A one-off backfill CLI (since deleted) reconstructed history from saved-image dates, leaving its supporting port methods behind.
- **Banner rewording**: the feed now shows "N images checked over past X, of which M (Y%) match", reading examined/shown counts and the interval for the list actually served (respecting fallback-feed widening). Retired the old `saved × 500` estimate and its entire read path (`estimate_processed`/`SAVE_RATE_PERCENT`/`v3-examined-count`).
- **Banner/grid consistency as a publisher invariant**: the publisher writes a live `exists` count (via the shared `is_live` predicate reused from its render filter) alongside `found`, so the banner number and the rendered grid can't drift; the feed became a dumb formatter displaying `exists` verbatim. Enforced with a publisher-side test.
- **Next-match prediction**: the publisher models matches as a homogeneous Poisson process (`λ = exists / window`) and publishes 2.5/50/97.5% waiting-time quantiles via the exponential inverse-CDF; zero-count windows emit no prediction. The feed re-anchors these to render time (Poisson memorylessness), shows a 95% range, and runs a live JS countdown that reloads at the median. Deliberately plain — no Bayesian or parameter-uncertainty machinery.
- **R2 read-amplification follow-up**: minute-ly stats writes raised R2 class-B ops (single-row Lance fragments re-scanned per publish). Mitigated by de-duplicating the publisher's per-spec reads by window and batching the pruner's appends over ~10 minutes; deeper consistency-tuning left cross-referenced to a later slice.
