...
## Slice: robust, low-impact firehose consumption + `skeet-prune` review/re-org

A maintenance-and-robustness slice on `skeet-prune`: a full human pass over the crate plus three firehose-consumption fixes so the pruner survives disconnects without losing observed posts and stays a polite client during outages. New workspace deps pulled into the crate: `tokio-util` (cancellation), `async-channel` (MPMC), `backon` (retry).

- **Shape/maintenance pass**: tightened the public surface (`pub mod` → `mod` + curated `lib.rs` façade); grouped the four ordered stages into a `pipeline/` module so the file tree shows the topology; killed the 12-positional-arg `PruneMetrics::emit` in favour of a neutral `PipelineSnapshot` struct (shaped to be reused by the upcoming statistics slice).
- **Counting redesign**: replaced the `Post`-marker double-send scheme with a `(content, counts)` writer-carrier shape where `ContentCounts` is a monoid the sink folds — collapsing the meta double-send and lifting the marker variant out of both content enums. Monoid laws proptested.
- **Shutdown seam**: unified the hand-copied closed-channel `is_err()` blocks onto a shared `CancellationToken` (`forward`/`recv` helpers).
- **Cursor-based resumption (in-memory only)**: track each event's `time_us` and set a rewound `JetstreamConfig.cursor` on reconnect so gaps replay losslessly (safe because the store is idempotent). A restart intentionally resumes at live-tail. Pure helpers (`event_time_us`/`cursor_from`/`replay_cursor`) extracted and proptested.
- **Reconnect backoff**: drove reconnects through `backon` (jittered, capped, never-give-up) wrapping our endpoint-rotating `connect`, replacing the no-delay retry-storm; a pure `session_was_stable` seam decides reset-vs-escalate.
...
An operational slice driving the existing eval/train bins (`capture-appraisals`, `refine-eval`, `sample-costs`, `train`, `promote`) against a grown ~1400-appraisal dataset, staying on `gpt-4o` and tuning only the prompt. Ended with a fresher prompt (`v2:ac1dde52`) serving in production. No new crates.

- **Fresh snapshot + re-baseline surfaced a calibration shift.** Captured a new `default` eval split and re-baselined the deployed model on it: precision fell sharply at the deployed threshold while ROC-AUC held — discrimination intact, calibration/distribution the problem (the ceiling owned by the "improving prune and refine quality" slice). Likely driver: the appraisal queue is fed by the model's own candidates, enriching it with hard negatives.
- **Control ruled out confounders.** Pinned the old split under a durable `frozen-143` label and re-ran the current model against it; it reproduced the historical numbers within run-to-run noise, ruling out gpt-4o API drift, eval-code, and price changes — the drop is genuinely the new distribution, so the incumbent is no longer good enough on the fuller set.
- **Pre-registered training to avoid optional-stopping bias.** Rather than "re-run if borderline" (which inflates false-accepts), fixed an N=3 protocol up front — including a non-max deploy pick — via a new `train-replicated` recipe. Two of three candidates passed the relative gate; adopted the median, a marginal Pareto win (an operating-point shift, not a discrimination gain).
- **Coordinated reader+writer deploy.** Since `config/refine.toml` is baked into both `live-refine` (score writer) and `skeet-publish` (score reader, which filters to its known version set), rolled both — otherwise the feed would silently discard the new model's scores. Patched the `docs/versioning.md` cutover checklist to record this, and updated the production-version tripwire test.
- **Fed learnings forward.** Refreshed the embeddings slice with the corrected dataset size, the pinned `frozen-143` reference, and confirmation that calibration (not discrimination) is the binding constraint.

## Slice: drain the pipeline on SIGTERM (close the restart in-flight loss)

The firehose slice made *reconnect* gaps lossless via a cursor but left *restart* loss untouched: shutdown was purely reactive (stages unwound only when a channel closed), so a k8s redeploy hard-killed the pruner with ~130 buffered items plus in-flight work silently lost. This slice makes a termination signal drain what's already in the pipeline into the idempotent store before exit. No new crates.

- **Two shutdown signals with distinct intent.** The pre-existing shared `CancellationToken` means *abort now* at every `recv`/`forward` (drops buffered work), so it kept that role — renamed `abort` — and a **separate `drain` token** was added that stops only the firehose source. On drain the source returns and drops its sender, and the bounded channels **close-cascade**: each stage drains what's buffered ahead of it, finishes, and drops its own output sender to close the next channel, so everything already in the pipeline lands in the content-hash-idempotent store.
- **Weak monitors were the load-bearing fix.** The channel-depth monitors had held strong `Sender` clones, which pinned the channels open — the reason the token had been the only viable shutdown path. Switching them to `WeakSender` (still fine for `.len()`) let the drop-cascade actually close channels without affecting reactive-abort detection.
- **Supervised, bounded shutdown.** `main` now spawns all five stages into a `JoinSet` and awaits them (not just the sink), racing a drain deadline that starts only once `drain` trips (`--drain-timeout-secs`, default 25, kept under the k8s grace period). A second signal escalates drain → abort. Live-verified: one Ctrl-C drained a full backlog through the bottleneck stage before exiting; a double Ctrl-C aborted promptly.
- **Pipeline assembly extracted.** The channel/spawn/shutdown wiring moved out of the bin into a `pipeline::runner` module behind a `Pipeline<S>` type with a `run()` method (generic over any `Images + Statistics` store), shrinking the bin to setup + one call.
- **Logging fix (global).** A `Targets` filter had dropped each binary's own logs whenever its crate name differed from the listed library crates; `init_with_file` now always folds `{bin}=info` (via `env!("CARGO_CRATE_NAME")`) so a rename can't silently lose them.

## Slice: refactor, review and code minimisation, focussed on remaining crates

The "be able to walk away from it for a while" maintenance pass extended across the crates left untouched by the earlier `skeet-store` slice — the publish/web-facing crates (`skeet-publish`, `skeet-feed`, `skeet-appraise`), the ML/detection libs (`skeet-refine` et al.), and the shared/support libs (`shared`, `bluesky`, `observability`, `eval`). Per-crate best-practice structure, tighter public surfaces, and closing `String`/`f32` gaps where validated domain types belong. No new crates.

- **URLs as validated types, not strings**: http(s) URLs that were built by interpolation and passed as bare `String` now parse into `url::Url` once at the boundary. A new `shared::BaseUrl` (proves can-be-a-base at construction, exposes `join_path`) backs the several `bsky.app`/CDN/API URL builders, so a malformed URL fails fast and construction from validated inputs is infallible.
- **Provenance travels with values**: extended the store slice's `ModelScore` (score + producing version) across the scoring pipeline so a bare `Score`/`Band` appears only in generic numeric/ordering code; appraisals now route band + appraiser through `Appraisal` rather than positionally.
- **New domain newtypes**: `FeedGeneratorUri`, `ModerationLabel`, and validating constructors for `Did`/`RecordKey`/`ModelProvider`/`Purpose` (previously accepted any string). `feed_config` accessors return domain types instead of raw `String`.
- **Jetstream ingress lifted into `bluesky`**: the transport + record-interpretation helpers moved out of `skeet-prune` into a `bluesky::firehose` module, since `bluesky` owns "talking to Bluesky"; the reconnect/cursor/backoff robustness stayed in the pruner. Widened the crate's charter from AppView client to AppView + Jetstream ingress.
- **Typed errors + surface tightening**: replaced `type Err = String` and `Box<dyn Error>` in FromStr impls and config loaders with `thiserror` enums, and narrowed over-broad `pub mod` to `mod` + curated `pub use` façades across the touched crates.
- **Recorded, not pursued**: TypeState for pipeline assembly, zero-copy borrowing views, and combinator-style filter composition — noted as assessed-and-declined so they aren't re-raised.

## Slice: 1.0 final tweaks and improvements

Final polish for the 1.0 release: marked all crates at version 1.0, added a combined quality+recency ordering as the site default, improved grid layout, ran a documentation/style compliance pass, and added a README. No new crates.

- **`Order::QualityRecency` ordering**: added a standalone `quality,recency` order (deliberately not the reverse) that buckets by effective quality Band — honouring manual appraisal overrides — then sorts newest-first within a band, with normalized score and id as tie-breaks. The `{order}-{limit}` list-name split stays unambiguous since the token contains a comma, not a dash.
- **Made it the site default**: defaults now split by consumer — the Bluesky feed and appraiser default to `quality,recency-48h`, the website image grid to `quality,recency-12w`; the publisher emits `quality,recency-{48h,7d,4w,12w,1y}` so both fallback chains have coverage. Deployed to staging then production and verified, then stopped publishing the `quality-{limit}` lists (removing orphaned redis keys) while retaining the `Order::Quality` code capability.
