...
* [ ] **Add a `NormalizedScore` newtype** in `shared` (sibling to `Score`: holds an `f32` in `[0,1]`, rejects NaN/out-of-range, `Eq`/`Ord` via `total_cmp`, `From<NormalizedScore> for f32`). Pure data — no policy methods.
* [ ] **Add the normalisation policy** `normalize(score: Score, threshold: Threshold) -> NormalizedScore` (`skeet-publish/effective_band.rs`): the threshold-anchored transform `T_t(x) = 0.5·x/t` for `x ≤ t`, else `(0.5·x + 0.5 − t)/(1 − t)`. Lives in `skeet-publish` because it's the feed/quality calibration *policy* (per the rust "policy in the owning crate" rule). Property-test: monotone in score; `normalize(s,t) ≥ 0.5 ⟺ s ≥ t` (i.e. ⟺ `RefineModel::is_positive`); equals `f32::from(score)` (identity) when `t = 0.5`; result always in `[0,1]`.
* [ ] **Retype `Band::from_score` → `Band::from_normalized(NormalizedScore) -> Band`** (`shared/band.rs`): keep the `0.25/0.5/0.75` cuts and the monotone property test, just change the input type so a *raw* `Score` can never reach it. This replaces the original "delete `Band::from_score`" task — the mapping survives but the only fixed-`0.5` boundary now operates in normalised space (where `0.5` *is* the threshold by construction). The model-aware band is then `Band::from_normalized(normalize(score, t))`; the band-mapping property `from_normalized(normalize(s,t)) ≥ MedHigh ⟺ s ≥ t` is the unification check.
* [ ] **Make `image_effective_band` model-aware**: change it to resolve the score→band via the producing model — `image_effective_band(score, model_version, models, manual_image_band) -> Band` (manual override still wins; otherwise `Band::from_normalized(normalize(score, model.decision_threshold))`). Unknown `model_version` → treat as not-positive (the behaviour `image_score_is_positive` had: log + cap below MedHigh), so visibility is unchanged for stale models.
