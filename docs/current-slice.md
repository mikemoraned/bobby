...
* [ ] **Refactor `prices.toml` into a price-snapshot registry** so each run's `cost_usd` is interpretable later and so phase-4 candidates are compared against a single pinned snapshot. Today `update-prices` overwrites `eval/prices.toml`; a recorded `cost_usd` becomes unmoored from its prices the moment models.dev changes a number, and a phase-4 sweep that straddles a price update would compare apples to oranges. Same registry shape as `refine.toml` / `eval-splits.toml`:
    * **`eval/prices.toml`** — top-level `[labels]` + `[[snapshots]]`. Each `[[snapshots]]` carries `snapshot_id`, `fetched_at` (UTC), `source_url` (today's `https://models.dev/api.json`), and the per-model `input_per_million` / `output_per_million` map currently sitting at file root. Labels point at canonical snapshots (`current = "20260510T1706"`). The existing single-snapshot file becomes the first entry.
    * **`update-prices` behaviour change:** appends a new `[[snapshots]]` entry (skip if identical content to `current`'s prices) and moves the `current` label to it. Never mutates or deletes existing snapshots. The provenance comment that currently sits at the top of `prices.toml` moves onto each `[[snapshots]]` entry as a `note` field.
    * **`ModelPrices` API:** `PricesRegistry::load/save`, `PricesRegistry::by_label(&Label) -> Option<&Snapshot>`, `PricesRegistry::by_id(&SnapshotId)`, `Snapshot::cost_for(...)`. `ModelPrices::embedded()` becomes `PricesRegistry::embedded().by_label(&Label::current())`.
    ...
    * **Phase-4 sweep control:** `train.rs` and `refine_eval.rs` gain an optional `--prices-snapshot-id` flag (default = `current`). Phase 4 pins one snapshot for the whole candidate sweep so the "pick the cheapest accepted candidate" comparison is over a single price column, not whatever happened to be current at each run's wall clock. Without pinning, a long sweep can silently change its own units mid-run.
