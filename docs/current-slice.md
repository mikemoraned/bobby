...
* [ ] **Close the feed-visibility gap via a per-model lookup** so each stored score is interpreted with the threshold of the model that produced it (not a global hardcoded 0.5). `refine.toml` becomes a *history* of models keyed by `version()` hash, with a top-level `[labels]` map (e.g. `production = "<hash>"`) so exactly one model is live at a time. Score consumers look up the model by the `model_version` already stored alongside each score. Concretely:
    * **TOML format**: top-level `[labels]` map + `[[models]]` array; each `[[models]]` entry persists its `hash` and `hash_scheme` (see the hash-method task below) alongside its fields, and `RefineModels::load` rejects mismatch between the persisted hash and a recomputed `RefineModel::version()` under the declared scheme. Labels must reference known hashes and each label may map to at most one hash.
    ...
    * `RefineModels` API: `load(path)`, `save(path)`, `get(&ModelVersion) -> Option<&RefineModel>`, `by_label(&Label) -> Option<&RefineModel>`, `insert(model, labels: &[Label])`. Errors as a `thiserror` enum (`HashMismatch`, `UnknownLabelHash`, `DuplicateHash`).
    * `train.rs`: on Accept, load existing `RefineModels` (or fresh), append the new entry, reassign `production` to the new hash, save. Don't overwrite history.
    ...
    * `refine-eval`: defaults to `production`; phase 4 will likely want a `--label` / `--hash` flag for non-production candidates — leave that for then.
    ...
    * **Migration of `config/refine.toml`**: backfill all model_versions that have live scores in R2 (see the backfill task below) so historical scores remain interpretable, and label the current `34d8bec0` entry `production`. Any model_version found in the store but unrecoverable from git history degrades to not-visible.
    * **Tests**: replace `tests/model_version.rs` with a `tests/refine_models.rs` load test (verifies `RefineModels::load(config/refine.toml)` succeeds and `production` resolves to `34d8bec0`). Unit tests for the three load-error modes and roundtrip. A feed-visibility test exercising lookup + unknown-`model_version` fallback.
* [ ] **Make the hash method a per-entry property of `RefineModel`** (following the `ImageId` `V1`/`V2` versioning idea, but without rewriting any stored hashes) so old hashes in storage remain round-trippable alongside new ones. Concretely:
    * Each `[[models]]` entry in `config/refine.toml` declares its `hash_scheme` (e.g. `"v1"` or `"v2"`) alongside its persisted `hash`. `RefineModel` carries the scheme as an enum field (e.g. `enum HashScheme { V1, V2 }`).
    * `RefineModel::version()` runs the algorithm named by its `hash_scheme`:
    ...
    * `RefineModels::load` recomputes each entry's hash under its declared scheme and verifies the match. Drift detection works for both schemes; no per-entry opt-out needed.
    * `shared::ModelVersion` stays as a thin `String` wrapper — no enum surgery, no prefix conventions. Score lookups happen by hash string equality regardless of scheme.
    * **No store migration**: existing scores tagged `34d8bec0` stay tagged `34d8bec0`. That entry lands in `refine.toml` with `hash_scheme = "v2"`, `hash = "34d8bec0"`, and self-verifies under V2 on load.
    * Tests: round-trip a `RefineModels` containing both V1 and V2 entries; hash-mismatch test per scheme; verify a V1 entry's recomputed hash ignores `decision_threshold`.
* [ ] **Backfill historical models into the new `refine.toml` format** so prior-prompt scores already in the store remain interpretable rather than degrading to not-visible. Steps:
    ...
    2. Walk `git log -p config/refine.toml` and `skeet-refine/tests/model_version.rs` to recover the `(hash, model_provider, model_name, prompt)` tuple for each historical revision. The test file records the canonical hash at each commit; `refine.toml` at the same commit carries the prompt.
    ...
    4. For each backfilled entry: set `decision_threshold = 0.5` (the implicit threshold those scores were produced under) and `hash_scheme = "v1"`. `RefineModels::load` verifies under V1 (which ignores `decision_threshold`), so the recomputed hash matches the original.
    5. Write the backfilled entries into `config/refine.toml` alongside the current `34d8bec0` (V2) entry, with `production` pointing at `34d8bec0`.
