...
* [ ] **Prune disappeared posts/images, and fold the dimension fetch into the same probe.** Some images are missing on the CDN and clicking through shows "Post not Found". The publisher (which already resolves CDN URLs) gains a live existence check, records the result as new flags on a `v3` `PublishedImage`, and readers either filter on them (`skeet-feed` feed + home) or annotate with them (`skeet-appraise`). Because the CDN existence probe is a header-read GET, it also yields the image's pixel dimensions — so we carry `image_url_dimensions` on the same `v3` pair and **delete `skeet-feed`'s `DimensionCache`** (module, layer, extractor, startup prefetch). The dimensions move that was rejected earlier "to avoid the schema bump" (see the aspect-ratio task above) is free now that `v3` bumps the schema regardless.
  ...
  * [ ] **Wire into the publisher** (`publisher.rs`): hold `Arc<dyn ExistenceChecker>`. After computing the candidate pairs for all specs (the union of skeets/urls, so shared entries are probed once), enrich each `PublishedImage` with its flags + dims from the checker, then `replace`. The change-gate is unchanged.
  * [ ] **CLI** (`bin/skeet_publish.rs`): construct the real checker; add `--existence-ttl-secs` (default ~3600) and `--existence-check-concurrency` (default ~16). `--once` and tests use the no-network double.
  * [ ] **Readers filter** (`source.rs`): `RedisFeedSource` reads `v3` automatically via `SCHEMA_VERSION`. Keep the raw `published()` **unfiltered** (appraise needs every item), but have both trait impls — `FeedSource::skeleton` and `PublishedImagesSource::published_images` — drop any item where `!image_url_exists || !skeet_id_exists`.
  * [ ] **`skeet-feed` home** (`handlers.rs`, `project.rs`, `bin/skeet_feed.rs`, delete `dimension_cache.rs`): remove the `DimensionCache` module/layer/extractor and the startup prefetch; `home` reads `image_url_dimensions` straight off each (already-filtered) `PublishedImage` for the `aspect-ratio`.
  * [ ] **`skeet-appraise`** (`feed_snapshot.rs`, `templates/home.html`): carry `image_url_exists` / `skeet_id_exists` through `FeedItem` (still dropping only items with no current score) and render a small per-card indicator when either is false — appraise shows everything, just badged.
  * [ ] **Tests**: `v3` serde/round-trip (incl. testcontainers `redis_published_list`); checker TTL + fail-open units; a publisher test injecting a double that marks some entries missing → correct flags written; `source.rs` tests that the two trait impls filter while `published()` does not.
