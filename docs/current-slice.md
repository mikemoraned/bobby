...
* [ ] update `skeet-appraise` so that `home` is able to select amongst all of the possible redis feeds available i.e. it can load `quality-7d`, `quality-48h`, and `recency-48h`. Make this a drop-down select at top of page, but choose `quality-48h` as the default.
