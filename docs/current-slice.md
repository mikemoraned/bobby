...
* implement the end of page experience in steps:
    ...
    2. [x] Then, in the publisher, do the proper maths i.e. treat this as a Poisson process and calculate confidence intervals of when, for each bucket of time (48h, 7d, 1y) and given current time, over what period of time we have a 95% chance of seeing another example of what we are looking for. Add this to the `:statistics` JSON as a few keys showing lower,middle,upper timepoints predicted. This prediction should take account of all arrival times seen during the period of interest. This should extend the `Statistics` port to implement these stats as needed.

        **Done differently than written — `λ = exists / window`, no `discovered_at`, no `Statistics`-port extension** (`prediction.rs`'s `predict_next_match`, called in `publisher.rs`'s `publish`; the quantiles ride `ListStatistics.next_match_prediction` as an `Option`, omitted from the JSON when there's no match — a covariant add, no `SCHEMA_VERSION` bump). The estimator the doc itself fixes — `λ = count / window` — uses only the *count* in the window and the window length, never individual arrival timestamps, so the `discovered_at`-vs-`original_at` clock can only change *which items fall in the window*. And the countdown predicts "next image appearing **in this bucket's list** on reload"; that list is bounded by `original_at` (`published_for_spec`'s `now − window` cutoff), so a bucket's arrival process is exactly the items entering it = `is_live` items with `original_at` in window = the already-computed `exists`. An old post freshly *discovered* never appears in the 48h list, so it correctly doesn't count toward the 48h rate. Hence `exists / window` is the right rate; threading `discovered_at` or adding a store method (which couldn't compute `is_live`/visibility anyway — both are publisher-side) would be machinery for an identical number. The `discovered_at` caution still applies to anyone computing `λ` from inter-arrival *timestamps* — we don't.

        **The model (deliberately kept simple — plain Poisson, no Bayesian/Lomax).** Throughout this step `now` means the **end of the current interval** (`interval_end`), not wall-clock call time — that's the origin we predict forward from.

        (Considered and rejected: anchoring `now` at the newest published item's timestamp instead — which can lag `interval_end`. By the **memorylessness** of the Poisson process the residual wait from `interval_end` is `Exponential(λ)` *regardless of how long ago the last match arrived*, so `interval_end` — the most recent time our observation actually covers — is the right origin. Anchoring at the last arrival would double-count the already-observed gap `interval_end − last_arrival`, a stretch we *know* contains no match, biasing every `now + T(p)` timepoint earlier and making the countdown expire too soon. The gap being "some time ago" is precisely the size of that bias, which is why we discard it rather than predict from it.) Treat matches as a homogeneous Poisson process with rate `λ = count / window`. The only fact needed: the time from *now* to the next match is `Exponential(λ)` (memoryless — independent of how long since the last one). The exponential CDF is `P(seen within t) = 1 − e^(−λt)`; invert it for "time by which there's a probability `p` of a match":
    ...
    3. [x] Update the feed website to use the middle timepoint as it's predicted time of next arrival, and to show the 95% confidence range (in text). Phrase the `upper` bound as the "95% chance by" point. When the publisher emitted no prediction (zero-count fallback above), show "not enough recent data" instead of a countdown.

        Phrased as a range per request: "Next one expected between {lower} and {upper} from now" (top) and an end-of-feed line "…expected sometime between {lower} and {upper} from now (95% confidence) — reloading {countdown}", the live JS countdown ticking to (and reloading at) the `middle`. The published quantiles are **re-anchored to render time** in `build_next_arrival`: the publisher measures them forward from `interval_end`, and by the Poisson memorylessness the wait from "now" has the same distribution, so the feed re-adds each wait to the current time — this stops the countdown starting already-expired when the page is served (or cached) some time after the publish. No-prediction (zero-match) windows show "isn't enough recent data to predict the next one yet" with no countdown. (The bare zero-match case also has an empty grid, which keeps its own "No selfies found just yet" message; the not-enough-data line covers the rare cards-present-but-no-prediction race.)
...
* [x] **De-duplicate the publisher's per-spec reads by `Limit`.** Only the `Limit` (window) affects the prune-stats read — the `Order` doesn't — so the 7 specs collapse to one read per *distinct* `Limit` (4 in today's set: 48h/7d/4w/1y, the `recency`/`quality` pairs sharing each window). In `publish()` (`publisher.rs:291-308`), before the per-spec write loop, gather the unique `Limit`s, call the existing `prune_stats_for_interval(now - limit.window(), now)` once per unique `Limit`, and stash the results in a `HashMap<Limit, PruneStats>` (or just the `images_examined` count). The per-spec loop then looks up its `Limit` in the map instead of querying. Cuts the reads from 7 → ~4 with no new port method and no in-memory window arithmetic — cleaner than reading the widest window and re-summing sub-windows by hand (which couples the publisher to the per-interval row shape). Less of a saving than a full collapse, but simple and obviously correct.
* [x] **Batch the appends over 10 minutes (addresses the root cause).** Keep aggregating the actual data at one-minute granularity — still produce one `PruneStats` per minute — but don't write each one individually. Buffer them in `StatisticsPersister` and flush every ~10 minutes via a new batch-accepting `record` variant on the `Statistics` port that takes a slice (`&[PruneStats]`) and writes them in a single append. This keeps the desired per-minute granularity in the stored rows while writing one multi-row fragment + one manifest bump per 10 minutes instead of ~60/hour — a ~10× cut in A's fragment count and C's write churn, with no change to the stored data model or read queries. (Flush on the cadence and on shutdown so the final partial batch isn't lost.)

##### Relevant planned work (`docs/next-slices.md`)

Two later-slice items bear on this, plus one that looks relevant but isn't:

* **Strong-consistency read tuning** (in *`skeet-store` engine & storage scaling*) is the planned home for Hypothesis B's fix: it proposes surfacing `read_consistency_interval` (today store-wide `Duration::ZERO`) per-CLI through `StoreArgs` so read-mostly paths tolerate a few seconds of staleness for fewer manifest re-reads. That's exactly the lever for B (the per-tick `version_snapshot()` manifest resolves) and the read half of C. Cross-referenced so the two don't drift — if we do the consistency knob here, tick that item there (and vice-versa).
* **Replay-based regression testing** is the guardrail that would have *caught* this automatically: its capture→replay→snapshot→diff over `r2.operations` is built for exactly this (its motivating incident is the 26 Apr R2 class-A blowup). The minute-ly `prune_stats` churn would make a good fixture/assertion when that slice lands.
* **Not the lever: aggregation/DataFusion pushdown** (Engine pushdown / DataFusion-direct items). Pushing the `prune_stats_for_interval` SUM into the scan reduces *bytes and compute*, but the scan still opens every matching fragment data file — so it doesn't cut the class-B *op count*, which is what hurts here. Fewer fragments (the batch task) and fewer manifest re-reads (consistency tuning) are the op-count levers; pushdown is orthogonal. (DataFusion-direct *could* help collapse the publisher's 7 per-spec reads into one `GROUP BY` query — but the in-memory aggregation in the collapse task above is simpler and needs no upgrade.)
