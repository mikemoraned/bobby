...
* [ ] tweak to publisher: when checking if it should publish a list it should check if the list currently exists in redis, as well as if anything has changed recently in the store. This is to cover the case where the redis list is deleted "behind the back" of the publisher and it still needs to recreate it, even if nothing has changed locally in the store since last it looked. Do this is in a test-first fashion i.e. first write a test which recreates the situaion and see it fail, then fix it.
