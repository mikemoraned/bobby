...
* [ ] **Verify**: confirm the machine actually suspends and resumes correctly serving `getFeedSkeleton` after an idle period; `just end_to_end_test_feed_staging` green; `just clippy` / `just test-no-docker`. *(Deployed to `bobby-staging` reading `v2-quality-48h`; `just end_to_end_test_feed_staging` green, `clippy`/`test-no-docker` clean. **Still to observe:** an actual idle suspend→resume cycle — the only path that exercises the new `backon` retry on the first post-resume read.)*
