...
* `skeet-publish`:
    ...
    * handles:
        ...
        * publishing one redis list per **(order, limit)** choice, named `{order}-{limit}` — so the Bluesky feed is `recency-48h` and the public list (Phase 5) is `recency-7d`. `Order` is an enum (only `Recency` today = by skeet publish time; `Quality` = by band/score, later); `Limit` is a `NewType(Duration)` rendered `48h`/`7d`/`365d`. The publisher is configured with a *set* of these specs and computes/writes each generically; a reader just reads the named list it wants. Lists are named by ordering+window, deliberately **not** by visual use (no `grid`/`feed`).
...
* [x] **Appraise home = published list (set+order) ⋈ targeted store detail.** In `skeet-appraise/handlers.rs::home`, replace `visible_entries(&CachedFeed)` with: read the `recency-48h` `PublishedPair`s via `RedisFeedSource` (refactor `skeleton` to extract a reusable "get pairs" method), then fetch score + skeet/image appraisals for **exactly those `image_id`s** from the store (a targeted lookup, **not** `FeedCache`'s capped bulk fetch — a published item outside the cap would render blank). Appraise then needs `BOBBY_REDIS_PUBLISH_URL` (read-only) in addition to its admin redis. *(Done — `RedisFeedSource::published()` extracted (skeleton dedups it). The join logic lives in a new **`feed_snapshot.rs::FeedSnapshot`** model (read published → targeted `list_scores_for_ids` + appraisal maps → join → effective band), so `home` is a thin view that maps `FeedSnapshot` to `HomeEntry`. `FeedCacheLayer`→`PublishedFeedLayer` (over `RedisFeedSource`); appraise bin drops `FeedCache`/models/`--model-path`/`--max-entries`/`--max-age-hours` and gains required `--redis-publish-url`. Deploy plumbing updated: `bobby-appraisals-staging.env` + `bobby-local.env` add `BOBBY_REDIS_PUBLISH_URL`; `fly.appraise-staging.toml` + `Dockerfile.fly` drop `--model-path`/refine.toml; `just appraise` now `op run`s for the redis url. Tests: home is a `_docker` test (publishes a matching item); `appraise_integration` reworked to a redis container (`_docker`); other appraise tests use a dummy redis url. clippy clean; `just test-no-docker` 473 passed.)*
* [x] **Delete `LiveFeedSource`** (no users once D1 lands) **and the `publisher_equivalence` test** (its comparison target, the library path, is gone). Rework `skeet-feed/tests/feed_endpoints.rs` to use a test-double `FeedSource` (or `RedisFeedSource`) instead of `LiveFeedSource`. Keep `visible_entries`. *(Done — `LiveFeedSource` removed from `source.rs` + lib export; `publisher_equivalence.rs` deleted (its `hardcoded_cids_are_valid_and_distinct` fixture check moved to `publisher_gating.rs`). `feed_endpoints.rs` rewritten around a `StubFeedSource` — kept the HTTP-surface tests (did.json, describe, serves-source-ids, max_entries cap, unknown-feed-400, last-modified); deleted the store-visibility tests (that policy lives + is tested in `skeet-publish`). `skeet-feed` dev-deps trimmed (`async-trait` added; `test-support`/`tempfile`/store `test-helpers` dropped). Also **deleted `FeedCache`/`CachedFeed`/`RefreshOutcome`** (`feed_cache.rs` removed) now that nothing uses them — the `FeedData` trait's only implementor is the publisher's `WindowedFeed`.)*
* [x] tweak to publisher: when checking if it should publish a list it should check if the list currently exists in redis, as well as if anything has changed recently in the store. This is to cover the case where the redis list is deleted "behind the back" of the publisher and it still needs to recreate it, even if nothing has changed locally in the store since last it looked. Do this is in a test-first fashion i.e. first write a test which recreates the situaion and see it fail, then fix it. *(Done — `PublishedList::exists` (redis `EXISTS`); `FeedPublisher::publish_if_changed` now skips only when the store is unchanged **and** `all_lists_present`, else republishes. Test-first: `publisher_gating.rs::republishes_when_the_list_was_deleted_docker` publishes → `DEL`s the list key → asserts the next cycle republishes. **Caveat:** an empty list has no key, so a legitimately-empty feed republishes every cycle (cheap, and rare on a live feed). I couldn't run the `_docker` test here — run `just test`.)*
* [x] **Extract the shared table-version lazy-load.** `live_refine`, `skeet_publish`, and `skeet_appraise` each lazy-load tables gated on version changes; pull the common mechanism (cf. `table_watch` + the snapshot-compare in `FeedPublisher`, and `skeet-store`'s `cached_scores`) into a shared module/crate. *(Done — `skeet_store::VersionedCache<V, T>` (`versioned_cache.rs`): one entry of a value `T` remembered against a version key `V`, serving both value caches (`get`) and skip-if-unchanged gates (`is_current`, `T = ()`). Adopted at all three sites: `cached_scores` (`VersionedCache<u64, ScoresMap>`, replacing the bespoke `ScoresCache` struct), `FeedPublisher.last_relevant` (`VersionedCache<HashSet<Version>, ()>` over the relevant-table subset from `table_watch::relevant`, now returning an owned set), and `PollingBatchSource.images_gate` (`VersionedCache<u64, ()>`, preserving that a `None` images version always re-fetches). Caller owns the lock (`RwLock`) or `&mut self`.)*

###### E. Make `skeet-feed` suspendable — step 5

Most of this group was already settled by D1's storeless rework — `skeet-feed` has no store, no `FeedCache`, no background refresh, no timers, and `RedisFeedSource` already connects **fresh per call** (no pool). So the substantive remaining work is just a retry wrapper (E1), the `soft_limit` (E3), and the deploy/verify (E5).

* [x] **Resilient redis access — retry-on-failure**: `RedisFeedSource` already connects fresh per `skeleton()` call (no pool), so there's no stale socket to survive a resume — the "validating pool" option is moot (and contradicts `redis_client.rs`'s documented no-pool reasons: Upstash idle drops + untunable deadpool timeouts). What's left is hardening the *first* connect/read after a resume against a transient DNS/TLS hiccup: wrap `published()`'s connect+read in a small bounded retry (2–3 attempts, short backoff). Each attempt connects fresh, so it reconnects + re-auths for free. **Reuse `backon`** (already a dep in `skeet-refine` — `ExponentialBuilder` + `.retry().when(is_transient).notify(…)`, see `skeet-refine/src/refining.rs:147`) rather than hand-rolling or adding a new retry crate; pull it to `[workspace.dependencies]` when `skeet-publish` takes it on. *(Done — `backon` pulled to `[workspace.dependencies]` (both `skeet-refine` and `skeet-publish` now reference it via `workspace = true`). `RedisFeedSource::published` retries the whole connect+read+refreshed_at closure under `ExponentialBuilder` (200ms min, ×2, max 3 retries); `is_transient` retries only connection-dropped/refusal/timeout/IO redis errors — never a JSON decode or other logic error — with unit tests for each branch.)*
* [x] **Fast cold start**: lazy-load from redis on first request, preload nothing; every deploy invalidates the snapshot so the first post-deploy request is a real cold start. *(Already satisfied by D1 — `skeet-feed` is storeless: the source holds only a url + list spec, nothing is loaded at boot, and the first request lazy-connects to redis.)*
* [x] **`fly.staging.toml`**: `auto_stop_machines` is already `"suspend"` and the health check already hits `/.well-known/did.json` (served from `FeedConfig` only, no redis — already shallow). Remaining: add a `[http_service.concurrency]` block with `soft_limit = 5` (low for staging, small headroom for Bluesky's polling plus an overlapping manual/e2e request). *(Done — added the `[http_service.concurrency]` block (`requests`, `soft_limit = 5`, `hard_limit = 25`) with a comment + Fly doc links; confirmed effective in the E verify — the machine reached `STATE=suspended` after idle.)*
* [x] **Time + telemetry across the boundary**: use wall-clock for anything time-sensitive (timers pause, clock lags on resume); don't alert on metric/log absence across suspend. *(Already satisfied — D1 removed the background-refresh timer, so `skeet-feed` has no `tokio::time::interval`; `refreshed_at` is wall-clock written by the publisher and read from redis. The "don't alert on metric absence" half is a dashboard concern, no repo change.)*
* [x] **Verify**: confirm the machine actually suspends and resumes correctly serving `getFeedSkeleton` after an idle period; `just end_to_end_test_feed_staging` green; `just clippy` / `just test-no-docker`. *(Verified on `bobby-staging` reading `v2-quality-48h`. `fly machine list` showed `STATE=suspended` after idle; the first post-resume `getFeedSkeleton` returned a populated feed (Upstash read succeeded on resume). Logs confirm a true snapshot-resume — `proxy … Starting machine` → immediate `serving getFeedSkeleton`, with **no** `Starting init`/`skeet-feed starting` (those appear only on deploys, which invalidate the snapshot). **No** `transient redis … will retry` line → clean resume, so the `backon` retry stayed dormant (a safety net for the unlucky case; its logic is unit-tested). `just end_to_end_test_feed_staging` 4/4 green, `clippy`/`test-no-docker` clean.)*

#### Phase 4: make `quality` be the default sort order

Recency is a totally reasonable metric but sorting by quality (score) is more engaging, particularly if we only want to show a small set of of skeets in the feed. So, introduce a `quality` sort order alongside `recency`. This should work as:
* sort first by derived Band i.e. sort by `High` and then `MedHigh`
* then, within each of these Bands, sort by associated score

Note that, if we sorted by score alone, we may not get the same result, as some scores will have been overridden manually.
...
* **The sort band is override-aware (min-rule), matching the visibility policy.** Each entry's sort band = `min(manual skeet band, manual image band else score-derived band)`. Within a band, entries sort by `NormalizedScore` (see next bullet), highest first. This is why band-then-score ≠ score-alone: a manual override moves an item's *band* but not its *score*.
* **Scores are normalised per-model before sorting (cross-model comparability).** Raw `Score`s from models with different `decision_threshold`s aren't comparable — a `0.65` from a `t=0.5` model is well past its bar; a `0.65` from a `t=0.6` model is barely past it — yet a within-band sort by raw score ranks them equal (visibly wrong at model rollovers, when a 48h/7d window straddles two `model_version`s). Fix: a **threshold-anchored piecewise-linear normalisation** `T_t(x) = 0.5·x/t` for `x ≤ t`, else `(0.5·x + 0.5 − t)/(1 − t)` — the standard cheap transform for making differently-thresholded classifier scores comparable. It maps each model's `t` to `0.5`, stays in `[0,1]`, is monotone, and is identity at `t = 0.5`. *Limitation:* single-anchor heuristic (aligns the decision boundary, assumes linearity either side) — it guarantees two normalised scores are past their respective thresholds by the same fraction of headroom, not rigorous probability calibration. Proper calibration (Platt/isotonic) is the documented upgrade path: it needs labelled validation data + a stored per-model calibration curve, and would slot in by replacing `normalize`'s body with a learned monotone map producing the same `NormalizedScore` — no downstream change. Not worth it for a selfie feed now.
* **Normalisation unifies with the band mapping (no separate `score_band`).** The model-aware band the visibility/quality policy needs is just `Band::from_normalized(normalize(score, t))`: `T_t`'s inverse images of `0.25/0.5/0.75` are exactly the per-model band cut-points, so one normalised quantity feeds **visibility** (`normalized ≥ 0.5 ⟺ RefineModel::is_positive`), **banding**, and the **cross-model within-band sort**. `Band::from_score`'s fixed `0.25/0.5/0.75` cuts become *correct* once they operate in normalised space (where `0.5` always is the threshold), so we **retype it to `Band::from_normalized(NormalizedScore)`** rather than deleting it — the only surviving fixed-`0.5` boundary now lives in normalised space, which is the threshold by construction.
* **Fix the two-boundaries bug first (group 0).** Today visibility uses the model's trained `decision_threshold` (`image_score_is_positive`) while the score→band derivation uses `Band::from_score`'s fixed `0.5` MedLow/MedHigh cut. For a model with `decision_threshold ≠ 0.5` (e.g. `0.6` in `config/refine.toml`) these disagree — an item can show a `MedHigh` badge yet be hidden, and the new quality sort would inherit the inconsistency. The trained threshold is authoritative (it's the learned operating point — `skeet-refine` training/`refine_eval`), so the **band bends to the threshold**, not the reverse. Unify everything (visibility, quality sort, *and* the appraise display badges) on one **model-aware** effective band (via the normalisation above) before adding the new order. **Scope: Tier 1 + Tier 2** — this re-threads `RefineModels` back into `skeet-appraise` (partly reversing D2's model-less appraise).
...
* [x] **Add a `NormalizedScore` newtype** in `shared` (sibling to `Score`: holds an `f32` in `[0,1]`, rejects NaN/out-of-range, `Eq`/`Ord` via `total_cmp`, `From<NormalizedScore> for f32`). Pure data — no policy methods.
* [x] **Add the normalisation policy** `normalize(score: Score, threshold: Threshold) -> NormalizedScore` (`skeet-publish/effective_band.rs`): the threshold-anchored transform `T_t(x) = 0.5·x/t` for `x ≤ t`, else `(0.5·x + 0.5 − t)/(1 − t)`. Lives in `skeet-publish` because it's the feed/quality calibration *policy* (per the rust "policy in the owning crate" rule). Property-test: monotone in score; `normalize(s,t) ≥ 0.5 ⟺ s ≥ t` (i.e. ⟺ `RefineModel::is_positive`); equals `f32::from(score)` (identity) when `t = 0.5`; result always in `[0,1]`.
* [x] **Retype `Band::from_score` → `Band::from_normalized(NormalizedScore) -> Band`** (`shared/band.rs`): keep the `0.25/0.5/0.75` cuts and the monotone property test, just change the input type so a *raw* `Score` can never reach it. This replaces the original "delete `Band::from_score`" task — the mapping survives but the only fixed-`0.5` boundary now operates in normalised space (where `0.5` *is* the threshold by construction). The model-aware band is then `Band::from_normalized(normalize(score, t))`; the band-mapping property `from_normalized(normalize(s,t)) ≥ MedHigh ⟺ s ≥ t` is the unification check.
* [x] **Make `image_effective_band` model-aware**: change it to resolve the score→band via the producing model — `image_effective_band(score, model_version, models, manual_image_band) -> Band` (manual override still wins; otherwise `Band::from_normalized(normalize(score, model.decision_threshold))`). Unknown `model_version` → treat as not-positive (the behaviour `image_score_is_positive` had: log + cap below MedHigh), so visibility is unchanged for stale models.
* [x] **Collapse the two boundaries into one in visibility** (`visibility.rs`): rewrite `visible_skeet_ids` to compute each image's model-aware `image_effective_band`, then per skeet use the extracted `skeet_effective_band` (below) and `is_visible_in_feed()`. **Delete `image_score_is_positive`** — visibility now reads the band, not a parallel positivity check. (`RefineModel::is_positive` stays; it's the authoritative definition that `score_band` is built on and is still used by training/eval.) The set of visible skeets must not change for the `t = 0.5` models — assert via the existing visibility tests.
* [x] **Extract `skeet_effective_band`** from `skeet_visible_in_feed` (`effective_band.rs`): `skeet_effective_band(manual_skeet_band: Option<Band>, image_effective_bands: &[Band]) -> Option<Band>` is the `min` it already computes; rewrite `skeet_visible_in_feed` as `skeet_effective_band(...).is_some_and(|b| b.is_visible_in_feed())` so visibility and the quality sort can never drift apart.
* [x] **Tier 2 — re-thread `RefineModels` into `skeet-appraise`** so its display badges use the same model-aware band as the feed: re-add `--model-path` + load `RefineModels` in `skeet_appraise.rs`, inject it (project/middleware) to `admin.rs` + `feed_snapshot.rs`, and replace their `Band::from_score(score)` / old `image_effective_band(score, manual)` calls with the model-aware form (both already have the row's `model_version` — `admin` via `scored`'s `mv`, `feed_snapshot` via the `(score, model_version)` it currently drops). Restore the appraise deploy plumbing D2 removed: `--model-path`/`refine.toml` in `Dockerfile.fly`, `fly.appraise-staging.toml`, `bobby-appraisals-staging.env`/`bobby-local.env`, and `just appraise`.
* [x] **Audit out every raw-score→band derivation** once `from_score` is retyped: the compiler will flag each old `Band::from_score(raw_score)` call (admin.rs, feed_snapshot.rs — handled in Tier 2). Every such site must route through `Band::from_normalized(normalize(score, model.decision_threshold))` so no fixed-`0.5` raw boundary survives in any policy or display path. The retype makes this fail-to-compile rather than fail-silently.

###### A. Add the `Quality` order + sort (`skeet-publish`)

* [x] **Add `Order::Quality`** to `order.rs`: `Display`/`FromStr` for `"quality"`; extend the roundtrip/reject tests to cover it. Update the doc comment (drop the "only `Recency` exists today" framing).
* [x] **Implement the `Order::Quality` arm** in `published_for_spec` (`publisher.rs`): after the window+visibility filter (entries are already one-image-per-skeet), compute for each entry both its `NormalizedScore` (`normalize(score, model.decision_threshold)`) and its sort band via the group-0 path — `skeet_effective_band(feed.skeet_band(&skeet_id), &[image_effective_band(score, model_version, feed.models(), feed.image_band(&image_id))])`. Sort by `(band, normalized_score)` *descending* — band first (`HighQuality` before `MediumHigh`, via `Band`'s existing `Ord`), then `NormalizedScore` (its `Ord` is `total_cmp`) highest-first within a band. The `NormalizedScore` tiebreak is what makes the within-band order correct across models with different thresholds; a manual override moves the `band` but not the `normalized_score`. Window/visibility/URL-resolution are unchanged — only the comparator differs from `Recency`.
* [x] **Unit tests for quality ordering** (mirror the recency tests in `publisher.rs`): (a) High-band entry sorts above a MedHigh-band entry even when its score is lower; (b) within one band, higher score first; (c) a manual band override reorders relative to score-alone — directly exercises the spec's note; (d) with a `decision_threshold = 0.6` model, a `0.55` score bands as MedLow (below the feed) — guards the group-0 fix; (e) **cross-model within-band tiebreak**: two entries in the *same* band, one from a `t=0.5` model and one from a `t=0.6` model — the entry further past *its own* threshold (higher `NormalizedScore`) sorts first even if its raw `Score` is lower. This is the case raw-score sorting gets wrong; it's the regression guard for the normalisation decision.
...
* [x] **Layout**: ~~uniform aspect-ratio tiles~~ → switched to **CSS multi-column masonry** (`column-width` + `break-inside: avoid`; `img { width:100%; height:auto }`) so each image shows whole at its own aspect ratio, no `object-fit: cover` cropping. It's the only broadly-compatible, JS-free option that needs no stored image dimensions. Trade-off: column-major fill, so the quality order reads down columns rather than across rows. Native CSS masonry (`grid-template-rows: masonry`) would keep row order but isn't compatible enough yet; CSS-grid `row-span` would keep row order but needs width/height carried in `PublishedImage` — defer unless wanted.
* [x] update `skeet-appraise` so that `home` is able to select amongst all of the possible redis feeds available i.e. it can load `quality-7d`, `quality-48h`, and `recency-48h`. Make this a drop-down select at top of page, but choose `quality-48h` as the default. *(Config-driven like `skeet-publish`: appraise takes repeatable `--publish <order>-<limit>` args (parsed via a now-shared `skeet_publish::parse_spec`), builds an `AvailableFeeds` (ordered specs + a `HashMap<(Order,Limit), RedisFeedSource>`); the first arg is the default and the arg order is the dropdown order. `?feed=` selects; unknown/absent falls back to the default. Deployment passes `quality-48h quality-7d recency-48h`.)*
* [x] **`skeet-feed` home: per-image aspect ratio via an in-feed dimension cache.** The home grid is CSS multi-column masonry showing whole images, but it doesn't know each image's aspect ratio, so it reflows (layout shift) as images stream in. Give each `<img>` an exact `aspect-ratio: W/H` so space is reserved up front. *(Code-complete + unit/endpoint tested offline (`cache_only`); the live CDN fetch + startup prefetch path is exercised only at deploy/manual verification.)*
    * **Approach (chosen over the upstream alternative):** a small `DimensionCache` helper injected into `home` (like the other source extractors), holding a `RwLock<HashMap<image-url, (w, h)>>`. On render, `dimensions(url)` returns cached dims if known and otherwise **blocks** on a fetch (GET the CDN thumbnail, read header-only via `image::ImageReader::into_dimensions()` — no full decode, no new crate), caches, and returns it. Blocking (not background-spawn) is fine here: the page is loaded rarely / typically once, so few viewers would benefit from async warming.
    * **Startup prefetch to hide the cost:** on cold process start, `main` spawns a **background, bounded-concurrency prefetch** that reads the selected home feed (`quality-7d`) once and warms every image's dimensions. It does **not** block `cot::run` — the server (incl. `getFeedSkeleton`) starts serving immediately and the cache fills shortly after; any render that races the prefetch just blocks on its own misses. So the only *live* per-render fetch cost is for images discovered since boot (few). It only runs on a real process start (deploy/restart), since the cache survives Fly `suspend`/resume (RAM snapshotted). Prefetch failures are logged and ignored (those images fall back to lazy fetch).
    * **Why in-feed, not the publisher:** dims for a CDN url are immutable → a single-write, never-stale cache. Keeps `PublishedImage`/publisher schema unchanged and `skeet-feed` self-contained. (The upstream alternative — carry `w`/`h` in a `PublishedImage` v3 sourced from the store's already-decoded `DynamicImage`, zero fetch ever — is the more "correct" data flow and would pair with the v3 existence-flags work below; rejected for now to avoid the schema bump.)
    * **Layout caveat:** known aspect ratios remove layout shift but don't, on their own, buy a responsive *row-order* full-image layout without JS (grid `row-span` needs the rendered pixel height; justified flex rows need JS or accept tiny crops). So this keeps the column-major masonry and just adds `aspect-ratio`. Row-order is a separate, larger call.
    * **Follow-ups noted:** bound/evict the cache (currently grows with the rolling 7d set); tests use a fetch-disabled (`cache_only`) constructor so they stay hermetic.
* [x] **Prune disappeared posts/images, and fold the dimension fetch into the same probe.** Some images are missing on the CDN and clicking through shows "Post not Found". The publisher (which already resolves CDN URLs) gains a live existence check, records the result as new flags on a `v3` `PublishedImage`, and readers either filter on them (`skeet-feed` feed + home) or annotate with them (`skeet-appraise`). Because the CDN existence probe is a header-read GET, it also yields the image's pixel dimensions — so we carry `image_url_dimensions` on the same `v3` pair and **delete `skeet-feed`'s `DimensionCache`** (module, layer, extractor, startup prefetch). The dimensions move that was rejected earlier "to avoid the schema bump" (see the aspect-ratio task above) is free now that `v3` bumps the schema regardless.

  **Decisions (from review):**
    * **Cadence — only on store change.** The check runs *inside* the existing publish path, gated by the same table-version change as today; **no separate recheck timer**. Trade-off: in a quiet period with no new skeets, a post deleted on Bluesky stays flagged-present until the next store-driven republish. Acceptable because the firehose keeps the widest window churning. (The gate also already republishes when a list is missing.)
    * **Cost — cache with TTL.** Checking every entry every cycle is many external calls. Keep an in-process cache of results keyed by `SkeetId` (existence) and `ImageUrl` (existence + dims); only re-check an entry whose last check is older than a TTL. Dimensions are immutable for a CDN url, so once known they are kept forever and only the existence half is re-probed. Bounds the call volume even across frequent (60s) republishes; a deletion is seen at most one TTL late.
    * **Inconclusive check — fail-open.** A network error / timeout / rate-limit yields `exists = true` (keep showing), so a transient Bluesky/CDN outage never blanks the feed or grid. A *successful* `getPosts` that simply omits a post is a genuine `false`, not an error.
    * **One probe, two jobs.** The CDN check is a GET read header-only via `image::ImageReader::into_dimensions()` (exactly what `DimensionCache` does today) — it proves existence *and* yields `(w, h)` in one request, superseding the "a HEAD might be enough" idea. Skeet existence is a batched `app.bsky.feed.getPosts` (≤25 uris/call, same `public.api.bsky.app` host `skeet-prune` already uses): a uri present in the response exists, an absent one is gone.

  * [x] **`v3` `PublishedImage` schema** (`skeet-publish/src/published.rs`): bump `SCHEMA_VERSION` to `v3` (so `v3-*` keys never collide with a still-deployed `v2` reader) and add `skeet_id_exists: bool`, `image_url_exists: bool`, `image_url_dimensions: Option<Dimensions>`. Move `Dimensions { width, height }` out of `skeet-feed` into `skeet-publish` (serde-derived) so it's the shared published type; update the serde shape test. *(Done differently: `Dimensions` and `ImageUrl` ended up in a new shared `bluesky` crate, not `skeet-publish`.)*
  * [x] **Existence checker** (new module in `skeet-publish`): an `ExistenceChecker` trait (`Send + Sync`) returning, for a set of `(SkeetId, ImageUrl)`, an existence map per skeet and an `ImageStatus { exists, dimensions }` per url — consulting/refreshing the TTL cache with bounded-concurrency probes (batched `getPosts` for skeets, header-read GET for images), fail-open on error. A real `CdnExistenceChecker` (reqwest) plus a no-network test double (mirrors `DimensionCache::cache_only`). *(Done differently: lives in a new `bluesky` crate shared with the firehose pruner; skeet existence uses one `getPostThread` per skeet — which also runs the pruner's blocked/adult-label check — not batched `getPosts`. A deleted post returns `400 NotFound`, so a `4xx` (except `429`) is treated as gone while `429`/`5xx`/transport stay fail-open. The double is `StaticExistenceChecker`.)*
  * [x] **Wire into the publisher** (`publisher.rs`): hold `Arc<dyn ExistenceChecker>`. After computing the candidate pairs for all specs (the union of skeets/urls, so shared entries are probed once), enrich each `PublishedImage` with its flags + dims from the checker, then `replace`. The change-gate is unchanged.
  * [x] **CLI** (`bin/skeet_publish.rs`): construct the real checker; add `--existence-ttl-secs` (default ~3600) and `--existence-check-concurrency` (default ~16). `--once` and tests use the no-network double. *(Done differently: `--once` uses the **real** checker so a local one-shot run actually exercises the live probes; only the unit/integration tests inject the no-network double.)*
  * [x] **Readers filter** (`source.rs`): `RedisFeedSource` reads `v3` automatically via `SCHEMA_VERSION`. Keep the raw `published()` **unfiltered** (appraise needs every item), but have both trait impls — `FeedSource::skeleton` and `PublishedImagesSource::published_images` — drop any item where `!image_url_exists || !skeet_id_exists`.
  * [x] **`skeet-feed` home** (`handlers.rs`, `project.rs`, `bin/skeet_feed.rs`, delete `dimension_cache.rs`): remove the `DimensionCache` module/layer/extractor and the startup prefetch; `home` reads `image_url_dimensions` straight off each (already-filtered) `PublishedImage` for the `aspect-ratio`.
  * [x] **`skeet-appraise`** (`feed_snapshot.rs`, `templates/home.html`): carry `image_url_exists` / `skeet_id_exists` through `FeedItem` (still dropping only items with no current score) and render a small per-card indicator when either is false — appraise shows everything, just badged.
  * [x] **Tests**: `v3` serde/round-trip (incl. testcontainers `redis_published_list`); checker TTL + fail-open units; a publisher test injecting a double that marks some entries missing → correct flags written; `source.rs` tests that the two trait impls filter while `published()` does not.
  * [x] **Deploy ordering**: deploy the publisher first (it writes `v3`, leaving the old `v2` keys orphaned — optionally `DEL` them), then deploy the `v3`-reading `skeet-feed` / `skeet-appraise`. The existing empty/cold-state handling covers the window before the first `v3` write.

###### C. Caching + polish

* [x] **HTML caching**: set `Last-Modified` from the grid's `refreshed_at` and honour `If-Modified-Since` → `304` (same pattern as `getFeedSkeleton`), plus a short `Cache-Control: public, max-age=…`. Low-traffic + suspendable, so a small max-age + revalidation is the right trade. *(Done differently: the `304` conditional-GET lives in a new `web-support` crate (`not_modified_since`, datetime-compared at second granularity) and is reused by `skeet-appraise`'s annotated image — `getFeedSkeleton` itself only sets `Last-Modified`, no off-the-shelf tower middleware does this for dynamic responses.)*
* [x] **Images need no work from us** — they're `cdn.bsky.app` URLs with Bluesky's own long-lived caching; `loading="lazy"` + explicit tile sizing (via `aspect-ratio`) avoids layout shift. Our caching responsibility is just the HTML (and the inlined CSS rides with it). *(Already in place in the grid template.)*
* [x] **Empty / cold states**: render a tidy empty grid when the `quality-7d` list is missing/empty (e.g. right after a deploy before the publisher's first write), not an error. *(Already handled — the template's empty branch; a missing list reads as an empty list.)*

###### D. Verify

* [x] **Integration test** (testcontainers redis, `_docker`): seed a `quality-7d` list, `GET /`, assert cards render in order with the right CDN `src` and `bsky.app` hrefs; `Last-Modified` present; `If-Modified-Since` returns `304`. *(Cache-header behaviour covered by handler-level tests in `feed_endpoints`: cache-control, last-modified, and the 304 conditional-GET.)*
* [x] **Manual**: deploy publisher + `skeet-feed`, confirm the public grid shows a week of `band >= MedHigh` images, clicks land on the right skeets, and the page still serves correctly through a suspend/resume cycle.
* [x] `just clippy`, `just test-no-docker`, `just end_to_end_test_feed_staging`.
