...
* [ ] **Train an improved prompt (N=3 pre-registered replication).** Run `just train-replicated` (`gpt-4o`, three seeds, gated against the auto-selected production-model eval on this split) per the pre-registered protocol in the 2026-07-03 observation. Do **not** re-roll to shop for an accept — that's optional stopping. Adopt iff ≥2 of 3 pass; deploy the median-recall passing candidate; else the incumbent stands. (Full variance-aware gating — mean/CI, re-baseline replication, in-loop overfitting — remains the later "improving prune and refine quality" slice; this is the minimal unbiased version.)
