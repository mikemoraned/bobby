...
There are still many things I want to do, but I'd like to get to a 1.0 version, which is complete-enough to leave for a while if I need to.

I'd like to have:
1. `bobby.houseofmoran.io`: same underlying code as the staging version (`bobby-staging.houseofmoran.io`), but:
    * published on Bluesky as a feed called "Bobby" (i.e. not "Bobby Dev")
        * has a small inline blurb explaining what this is, which is shared with website (see below)
    ...
    * a small banner at top which shows:
        ...
        * a small qr code for `https://bobby.houseofmoran.io/` url
...
2. bobby-appraisals.houseofmoran.io: same underlying code as the staging version
  * nothing additional needed i.e. it's the same thing, just running under an additional url
3. a separation between production and staging setups to avoid accidental breakage:
...
4. refactor, review and minimisation of code for longer-term maintainance:
