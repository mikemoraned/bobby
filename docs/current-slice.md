...
* [ ] **De-duplicate the publisher's per-spec reads by `Limit`.** Only the `Limit` (window) affects the prune-stats read — the `Order` doesn't — so the 7 specs collapse to one read per *distinct* `Limit` (4 in today's set: 48h/7d/4w/1y, the `recency`/`quality` pairs sharing each window). In `publish()` (`publisher.rs:291-308`), before the per-spec write loop, gather the unique `Limit`s, call the existing `prune_stats_for_interval(now - limit.window(), now)` once per unique `Limit`, and stash the results in a `HashMap<Limit, PruneStats>` (or just the `images_examined` count). The per-spec loop then looks up its `Limit` in the map instead of querying. Cuts the reads from 7 → ~4 with no new port method and no in-memory window arithmetic — cleaner than reading the widest window and re-summing sub-windows by hand (which couples the publisher to the per-interval row shape). Less of a saving than a full collapse, but simple and obviously correct.
