...
* [ ] **Extract the shared table-version lazy-load.** `live_refine`, `skeet_publish`, and `skeet_appraise` each lazy-load tables gated on version changes; pull the common mechanism (cf. `table_watch` + the snapshot-compare in `FeedPublisher`, and `skeet-store`'s `cached_scores`) into a shared module/crate.
