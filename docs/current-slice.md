...
* [x] **Verify + document.** `just clippy` + `just test-no-docker`. Record the snapshot `split_id`, baseline run-id, candidate run-id, and outcome (accepted+promoted, or rejected) so the comparison is reproducible. (Updated the `refine_models` production-version tripwire test to `v2:ac1dde52`. Reproducibility ids: split `3ad0a0b…`; baseline run `019f282e`; candidate runs seed42 `019f285e`→`v2:ac1dde52` (adopted), seed43 `019f28d3`→`v2:1835e655`, seed44 `019f2a61`→`v2:4ff58310` (rejected); outcome: accepted + promoted + deployed.)
