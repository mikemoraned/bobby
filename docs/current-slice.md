...
* [ ] **Data/store layer — `skeet-store`.** The largest crate (~6.3k LOC); pay attention to module boundaries and the read/write paths. Run `just validate-storage` after.
    ...
    * **Cheap DRY cleanups (review §C / §E #10 / §7 / §2.3 / §5) — low-risk, reduce future-edit cost; fold in while already in the crate.**
        * [ ] **`FromRecordBatch` decode trait (§C).** ≈14 hand-rolled `0..num_rows` decode loops remain (`scored_view.rs`, `stored.rs`, `scores.rs`, `images.rs`, `appraisals.rs`) — Stage B *added* some (the new `scored_view.rs`). A small internal `FromRecordBatch` (or `rows(&batch) -> impl Iterator`) co-locates the column-name list with the struct it builds and removes the duplication.
        * [ ] **`TableName` enum (§E #10).** The 5 table names are `const &str` (`schema.rs:5-9`) flowing as `&'static str`/`String` through `version.rs` (`table_versions`/`fragment_counts`), `error.rs`, and metric labels — a closed set masquerading as open strings. An enum with `as_str()`/`Display` makes "iterate all tables" compiler-exhaustive and ends the `&'static str` vs `String` mixing. Drive-by (§5): while here, add a one-line comment on the schema `archetype` column noting it backs the Rust `Zone` field (or rename in a future `images_v7`).
        * [ ] **Declarative `TableSpec` + fold the `tables` registry (§7 / §2.3).** `open()` (~196 lines) repeats create-table-if-missing ×5 and create-index-if-missing ~6× (≈15 create/index calls), and the `tables` registry (`lib.rs`, populated in `open.rs`) duplicates the named fields. A declarative `[(name, schema_fn, &[indexed_columns])]` spec iterated once collapses both and makes "add a table" a one-line edit.
    * *Optional follow-up — not required to close the store pass:* typed predicates via `only_if_expr` (§3.1). 14 string-interpolated predicates remain, but the one real injection hole is already closed by typing `list_scores_for_ids`, so this is now a robustness + engine-pushdown enhancement (gateway to §3.3/§3.4 and the DataFusion-direct direction §10.2) — its own later effort, not a cleanup blocker.
