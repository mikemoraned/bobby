...
* [x] features:
  ...
  * [x] make the grid display in the feed show the top entries of the list further up the page. as it stands right now, we show the best entries first in a column that goes all the way to the bottom of the page before looping round to the top. We want to show the best content further up the page whilst still showing it as a grid, similar in style to what we have now.
    * approach chosen: keep the existing CSS multi-column masonry unchanged and reorder the cards server-side. `bias_top_by_columns` deals the best-first list round-robin across the assumed `HOME_GRID_COLUMNS` (4) columns and concatenates them (`0,1,…,9` → `0,4,8,1,5,9,2,6,3,7`), so a column-major fill lands the best across the top row. Bias is exact at 4 columns, approximate at other counts (a narrow/mobile viewport with fewer columns interleaves the next few but still leads with the best).
    * considered and rejected: (a) server-computed CSS-grid row spans (`grid-row: span N` from each thumbnail's aspect ratio) — gives true row-major masonry at any column count, but is more math, needs fixed-width columns, and estimates spans for unknown-dimension thumbnails; (b) a JS masonry that measures rendered heights — most robust but adds load-time reflow/jank and client JS. The round-robin deal was preferred as the simplest change that keeps the current layout.
    * future simplification (noted in the `bias_top_by_columns` doc comment): native CSS masonry (`grid-template-rows: masonry`, or the CSSWG's `item-flow` work) does row-major shortest-column packing at any column count with no JS or reordering — not stable cross-browser yet (Firefox behind a flag, Safari Tech Preview). When it ships unflagged, drop the reorder and order cards best-first directly.
* [x] checks docs / skills
