...
* [x] **Delete `LiveFeedSource`** (no users once D1 lands) **and the `publisher_equivalence` test** (its comparison target, the library path, is gone). Rework `skeet-feed/tests/feed_endpoints.rs` to use a test-double `FeedSource` (or `RedisFeedSource`) instead of `LiveFeedSource`. Keep `visible_entries`. *(Done — `LiveFeedSource` removed from `source.rs` + lib export; `publisher_equivalence.rs` deleted (its `hardcoded_cids_are_valid_and_distinct` fixture check moved to `publisher_gating.rs`). `feed_endpoints.rs` rewritten around a `StubFeedSource` — kept the HTTP-surface tests (did.json, describe, serves-source-ids, max_entries cap, unknown-feed-400, last-modified); deleted the store-visibility tests (that policy lives + is tested in `skeet-publish`). `skeet-feed` dev-deps trimmed (`async-trait` added; `test-support`/`tempfile`/store `test-helpers` dropped). **Now dead:** `FeedCache`/`CachedFeed`/`RefreshOutcome` have no remaining users — fold their deletion into the lazy-load extraction below.)*
