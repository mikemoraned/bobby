...
* [x] `crates/skeet-prune/src/bin/pruner.rs` has grown large — the pipeline assembly (channel creation, per-stage spawns into the `JoinSet`, signal + drain wiring) could move into a `pipeline` submodule (e.g. `pipeline::assemble`/`run`), leaving the bin as arg-parsing + config + a single call.
  * Done as `pipeline::runner::run_pipeline` (+ `PipelineSettings`), generic over any `Images + Statistics` store. Bin dropped 277 → 120 lines.
